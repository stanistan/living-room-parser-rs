...
Module.STDWEB_PRIVATE.acquire_tmp = function( dummy ) {
    ...
    return Module.STDWEB_PRIVATE.acquire_tmp(Module.instance.exports.parse(Module.STDWEB_PRIVATE.prepare_any_arg(s)));
}
;
    ...
    if( __load_asynchronously ) {
        return WebAssembly.instantiate( __wasm_module, __imports )
            .then( instance => {
                ...
                console.log( "Finished loading Rust wasm module 'parser'" );
                return Module.exports;
            })
            .catch( error => {
                console.log( "Error loading Rust wasm module 'parser':", error );
                throw error;
            });
    } else {
    ...
    if( typeof window === "undefined" && typeof process === "object" ) {
        ...
        const wasm_path = path.join( __dirname, "parser.wasm" );
        const buffer = fs.readFileSync( wasm_path );
        const mod = new WebAssembly.Module( buffer );

        return __initialize( mod, false );
    } else {
        return fetch( "parser.wasm" )
